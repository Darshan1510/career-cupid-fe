...
const ShortlistCandidates = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (job.applicants && job.applicants.length > 0) {
          const seekersData: any = await getSeekersByFilter(
            new URLSearchParams(`seekerIds=${job.applicants}`)
          );
  ...
  return (
    <>
      {lastJob && lastJob.applicants && lastJob.applicants.length > 0 ? (
        ...
        <h4 className="text-center mt-3">No candidates have applied yet</h4>
      )}
    </>
  );
};
